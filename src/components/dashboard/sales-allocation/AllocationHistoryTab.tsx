...
import { Eye, Ban, Download, Filter, Search, History } from 'lucide-react';
...
export function AllocationHistoryTab({ onRevoke }: AllocationHistoryTabProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [projectFilter, setProjectFilter] = useState('all');
  const [statusFilter, setStatusFilter] = useState('all');
  const [paymentFilter, setPaymentFilter] = useState('all');

  const filteredAllocations = mockAllocationHistory.filter(allocation => {
  ...
  const projects = [...new Set(mockAllocationHistory.map(a => a.project))];
