...
export function ProjectForm({ onClose, project, onFormChange }: ProjectFormProps) {
  ...
  useEffect(() => {
    ...
    const hasFormChanges = Object.keys(formData).some(key => {
      ...
      if (key === 'tags') {
        ...
        const originalTags = project.tags?.join(', ') || '';
        return currentTags !== originalTags;
      }
      return formData[key as keyof typeof formData] !== (project[key]?.toString() || '');
    });
    
    setHasChanges(hasFormChanges);
    onFormChange?.(hasFormChanges);
  }, [formData, project, onFormChange]);
  ...
  return (
    <div className="relative">
      ...
      {/* Fixed Submit Button */}
      <div className="fixed bottom-0 right-0 bg-white border-t p-4 w-full max-w-4xl mx-auto">
