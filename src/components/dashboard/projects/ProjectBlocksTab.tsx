...
export function ProjectBlocksTab({ project }: ProjectBlocksTabProps) {
  ...
  const handleBlockClick = (block: any) => {
    navigate(`/company/projects/${project._id}/blocks/${block._id}`);
  };

  const handleUpdateBlock = (updatedBlock: any) => {
    setBlocks(
      blocks.map((block) =>
        block._id === updatedBlock._id ? { ...block, ...updatedBlock } : block
      )
    );
  ...
  const totalUnits = blocks.reduce((sum, block) => sum + block.totalPlots, 0);
  ...
  const totalAllocated = blocks.reduce(
    (sum, block) => sum + (block.allocatedPlots || 0),
    0
  );
