...
export function ProjectSalesHistoryTab({
  project,
  onReallocate,
  onRevoke,
}: ProjectSalesHistoryTabProps) {
  ...
  const handleRevoke = (allocation: any) => {
    if (onRevoke) {
      ...
      toast.success(
        `Allocation revoked for ${allocation.unitId} from ${allocation.clientName}`
      );
    }
  };
  ...
  const fullAllocationSales = mockAllocatedUnits.filter(
    (unit) => unit.allocationStatus === "allocated"
  );
  const unallocatedSales = mockOfferedUnits.filter(
    (unit) => unit.offerStatus === "offered"
  );

  const allocatedStats = {
    totalAllocated: fullAllocationSales.length,
    totalValue: "₦55,800,000",
    fullyPaid: fullAllocationSales.filter(
      (u) => u.paymentStatus === "completed"
    ).length,
    conversionRate: (
      (fullAllocationSales.length /
        (fullAllocationSales.length + unallocatedSales.length)) *
      100
    ).toFixed(1),
  };
  ...
  const offeredStats = {
    ...
    pending: unallocatedSales.filter((u) => u.status === "Pending Response")
      .length,
    underNegotiation: unallocatedSales.filter(
      (u) => u.status === "Under Negotiation"
    ).length,
  };
                ...
                <TableBody>
                  {fullAllocationSales.map((unit) => (
                    <TableRow key={unit.id}>
                      ...
                      <TableCell>
                        <Badge
                          className={getPaymentStatusColor(unit.paymentStatus)}
                        >
                          {unit.status}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex space-x-2">
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              handleReallocate(unit.unitId, unit.clientName)
                            }
                          >
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  Are you sure you want to revoke the allocation
                                  of {unit.unitId} from {unit.clientName}? This
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  Are you sure you want to revoke the offer of{" "}
                                  {unit.unitId} to {unit.clientName}? This
                                  action cannot be undone.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
